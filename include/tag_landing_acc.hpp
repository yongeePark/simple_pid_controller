...
private:
    ...
    double last_tag_pose_[3];
    // ros::Time last_tag_time_;
    std::chrono::system_clock::time_point last_tag_time_;
    double tag_velocity_[3];

    double last_velocity_cmd_[3];
...
public:
    ...
    use_yaw_(false), is_landing_ready_(false),is_goal_received_(false),
    last_yaw_(0.0)//, last_cmd_x_(0.0), last_cmd_y_(0.0)
	{
        ...
        last_velocity_cmd_[0] = 0;
        last_velocity_cmd_[1] = 0;
        last_velocity_cmd_[2] = 0;
...
{
// Parameter Setting
    std::string nodename = "/tag_landing_acc";
    ...
    if (!nh_param_.getParam(nodename+"/acc_limit",acc_limit_))
    {
        std::cout<<"[Warning] Please set [acc_limit] parameter, default : 0.1"<<std::endl;
        acc_limit_ = 0.1;
    }
